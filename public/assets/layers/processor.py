...
def format_geometry(geometry):
    ...
    case = {
        ...
        # Uncomment below and comment the above line when the JSON is a Point
        # 'Point': "POINT(" + " ".join(map(str, coords[:2])) + ")",
        
        # Comment all below when the JSON is a Point
        'LineString': "LINESTRING(" + ", ".join(" ".join(map(str, coord[:2])) for coord in coords) + ")",
